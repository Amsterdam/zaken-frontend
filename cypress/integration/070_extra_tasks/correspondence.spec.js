...
describe('Add extra task "Correspondence', () => {
  ...
  describe("Submit in form 'Taak opvoeren'", () => {  
    ...
    it("Select Task Correspondence", () => {
      ...
      cy.get('[data-e2e-id="workflowProcess"]')
        .select(extraTasks.taskCorrepondence)
      ...
      cy.get(`[role="dialog"]`)
        .should("contain", extraTasks.taskCorrepondence)
        .find("button")
    ...
    it("Request is successfully processed", () => {
      ...
      cy.wait("@getEvents").then(() => {
        cy.get("h4")
          .contains(extraTasks.taskCorrepondence)
        cy.get("tbody>tr")
          .contains("td", extraTasks.taskCorrepondenceVerbose)
  ...
  describe("PHH processes Correspondence", () => {  
    ...
    it('PHH can finish task "Correspondence"', () => {
      ...
      cy.get("tbody>tr>td")
        .eq(1)
        .should("contain",extraTasks.taskCorrepondenceVerbose)
        .siblings('td')
        .contains(roles.PHH)
        .parents('td')
        .siblings('td')
      ...
      cy.get(`[role="dialog"]`)
          ...
          .contains("h4", extraTasks.taskCorrepondenceVerbose)
  ...
  describe("Check request is processed", () => {  
    ...
    it("History contains the right items", () => {
      cy.history(extraTasks.taskCorrepondenceVerbose, "Uitvoerder")
    })
  })
})
