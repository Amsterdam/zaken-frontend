...
const isGetOptions = (options: Options): options is GetOptions => options.method === 'get';

const isMutateOptions = (options: Options): options is MutateOptions => ['post', 'put', 'patch', 'delete'].includes(options.method);
...
const useApiRequest = <Schema, Payload = Partial<Schema>>({
  url, groupName, handleError, isProtected, lazy, keepUsingInvalidCache, isMocked = false, isMockExtended = false,
}: Config) => {
  const {
    getCacheItem,
    setCacheItem,
  ...
  const queueRequest = useCallback(async (options: Options, payload?: Payload) => new Promise(
    (resolve, reject) => pushRequestInQueue(url, options.method, () => execRequest(options, payload)
      .then(resolve)
      .catch(reject)),
   ...
   * Define HTTP methods
   */
  const execGet = useCallback((options?: Omit<GetOptions, 'method'>) => queueRequest({ method: 'get', ...options }), [queueRequest]);

  const execPost = useCallback((payload?: Payload, options?: Omit<MutateOptions, 'method'>) => queueRequest({ method: 'post', ...options }, payload), [queueRequest]);

  const execPut = useCallback((payload?: Payload, options?: Omit<MutateOptions, 'method'>) => queueRequest({ method: 'put', ...options }, payload), [queueRequest]);

  const execPatch = useCallback((payload?: Payload, options?: Omit<MutateOptions, 'method'>) => queueRequest({ method: 'patch', ...options }, payload), [queueRequest]);

  const execDelete = useCallback((options?: Omit<MutateOptions, 'method'>) => queueRequest({ method: 'delete', ...options }), [queueRequest]);
