...
type Action = { type: "PUSH"; item: QueueItem } | { type: "SHIFT" };
...
const isPending = (
  ...
  state.find(
    (_) => _.url === url && _.method.toUpperCase() === method.toUpperCase(),
  ) !== undefined;

export const useRequestQueue = () => {
  const [isBusy, setIsBusy] = useState(false);
  const [state, dispatch] = useReducer(reducer, []);
  const isRequestPendingInQueue = useCallback(
    (url: string, method: string) => isPending(state, url, method),
    [state],
  );
