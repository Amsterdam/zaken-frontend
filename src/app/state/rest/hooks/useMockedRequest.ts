...
const timeout = async (ms: number) => await new Promise(resolve => window.setTimeout(resolve, ms))
...
export default () => useCallback(
  async <Schema>(method: Method, url: string, requestData?: unknown, headers = {}) => {
    ...
    const data = method === "get"
      ? urlId !== undefined
        ? ((mockData[urlId[0] as MockDataKey] as any[])?.find((item: { id: number }) => item.id === urlId[1]) as Schema)
