...
describe("useRequestQueue", () => {
  ...
  it("execute pushed requests in order", async () => {
    ...
    expect(result.current.isRequestPendingInQueue("foo1", "GET")).toEqual(
      false,
    ); // <- First request is not pending anymore
    ...
    await waitFor(() => {
      expect(result.current.isRequestPendingInQueue("foo2", "GET")).toEqual(
        false,
      ); // <- Both are NOT pending anymore
    });
  ...
  it("does NOT push duplicate requests", async () => {
    ...
    await waitFor(() => {
      expect(
        result.current.isRequestPendingInQueue("duplicate", "GET"),
      ).toEqual(false);
    });
