...
export default (
  ...
  const fields = {
    ...
    identification: {
      ...
      props: {
        ...
        field: {
          ...
          props: {
            label: 'SIA-nummer',
            extraLabel: <InfoButton infoTitle="SIA-nummer" infoText="Vermeld hier het corresponderende SIA-nummer, zodat de melding makkelijk terug te vinden is in SIA." />,
            name: 'identification',
            placeholder: '123456',
            min: 1,
            step: 1,
    ...
    nuisance: {
      type: 'ShowHide',
      props: {
        shouldShow: (formValues: { values?: { reason?: Components.Schemas.CaseReason, theme?: Components.Schemas.CaseTheme } }) => formValues?.values?.theme?.name === 'Vakantieverhuur' && formValues?.values?.reason?.name === 'SIA melding',
        field: {
          type: 'CheckboxFields',
          props: {
            label: 'Betreft overlast',
            name: 'nuisance',
            extraLabel: <InfoButton infoTitle="Betreft overlast" infoText="Aanvinken indien in de melding sprake is van overlast zoals geluid, lawaai, stank en vuil overlast." />,
            options: {
              nuisance: 'Ja',
            },
          },
    ...
    advertisement: {
      type: 'ShowHide',
      props: {
        shouldShow: (formValues: { values?: { theme?: Components.Schemas.CaseTheme } }) => formValues?.values?.theme?.name !== undefined && formValues?.values?.theme?.name !== 'Kamerverhuur' && formValues?.values?.theme?.name !== 'Ondermijning',
        field: {
          type: 'RadioFields',
          props: {
            isRequired: true,
